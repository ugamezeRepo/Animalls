			...
			<section id="infoSection">
				...
				<div class="container">
					...
					<div class="mb-2">
						...
						<button type="submit" class="btn btn-primary" id="sendAuthEmailBtn" disabled>인증번호 받기</button>
					</div>
					
					<!-- 이메일 인증 -->
					<div class="mb-2" id="ChkEmailAuth">
						<label calss="form-label" for="emailAuth">인증번호</label>
						<input class="form-control" type="emailAuth" name="emailAuth" id="emailAuth" maxlength="6" />
...
=======
		...
		const chkPwd = () => {
			if (pwd.value != pwd2.value) {
				document.getElementById("pwdExText").innerText = "비밀번호가 일치하지 않습니다.";
		...
		pwd.addEventListener("blur", () => {
			if (!reg_password.test(pwd.value)) {
				document.getElementById("pwdExText").innerText = "비밀번호를 규칙에 맞게 작성해주세요.";
				pwd.classList.remove("is-valid");
				pwd.classList.add("is-invalid");
				pwd2.classList.remove("is-valid");
				pwd2.classList.add("is-invalid");
		...
		email.addEventListener("blur", e => {
			...
			fetch("${pageContext.request.contextPath}/member/check_email.jsp?email=" + email.value)
			.then(res => res.json())
			.then(data => {
				email.classList.remove("is-valid", "is-invalid");
				if (data.canUse) {
		...
		sendAuthEmailBtn.addEventListener("click", (e) => {
	        ...
	        fetch("${pageContext.request.contextPath}/member/check_authEmail.jsp?email=" + email.value + "&emailAuthCode=" + emailAuthCode)
            .then((res) => res.json())
            .then((data) => {
            });
